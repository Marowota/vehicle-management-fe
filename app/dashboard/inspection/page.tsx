...
export default function InspectionPage() {
  ...
  const [vehicleInspects, setVehicleInspects] = useState<
    VehicleInspectionInfo[]
  >([]);
  ...
  return (
    <>
      <div className="px-10 py-10 h-full w-full flex flex-col ">
        <div className="font-bold text-xl text-blue-900">Đăng kiểm</div>
        <div className="border border-sky-100 my-3"></div>

        <div className="flex items-start h-1/12 w-full my-3 ">
          <div className="flex gap-4">
            {searchOption == InspectionSearchParams.REGISTRATION_DATE ||
            searchOption == InspectionSearchParams.VALID_UNTIL ? (
              <>
                <div>
                  <p>Từ ngày</p>
                  <DatePicker
            ...
            <div>
              ...
              <select
                ...
                value={searchOption}
                onChange={(e) => {
                  const params: InspectionSearchParams = e.target
                    .value as InspectionSearchParams;
                  setSearchOption(params);
                }}
              >
                <option value="VALID_UNTIL">Hiệu lực đến</option>
                <option value="REGISTRATION_DATE">Ngày đăng kiểm</option>
                <option value="PLATE_NUMBER">Biển số xe</option>
                <option value="INSPECTION_NO">Mã đăng kiểm</option>
              </select>
            </div>
            <div>
          ...
          <div className="w-full h-full flex ">
            <div className="relative overflow-auto shadow-md sm:rounded-lg w-full h-full  ">
              <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
                ...
                <tbody>
                  {vehicleInspects.map((vehicleReg, i): React.ReactNode => {
                    return (
                      <tr
                      ...
                      >
                        ...
                        <td className="px-6 py-4">{vehicleReg.inspectionNo}</td>
                        <td className="px-6 py-4">
                          {new Date(
                            vehicleReg.registrationDate.toLocaleString()
                          ).toLocaleDateString()}
                        </td>
                        <td className="px-6 py-4">
                          {new Date(
                            vehicleReg.validUntil.toLocaleString()
                          ).toLocaleDateString()}
                        </td>
